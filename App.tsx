...
import { generateEditedImage, generateFilteredImage, generateAdjustedImage, generateRemovedObjectImage } from './services/geminiService';
...
type Tab = 'retouch' | 'remove' | 'adjust' | 'filters' | 'crop';
...
const App: React.FC = () => {
  ...
  const lastMaskPos = useRef({ x: 0, y: 0 });
  ...
  // Effect to sync canvas size with the displayed image
  useEffect(() => {
  ...
  const handleApplyRemove = useCallback(async () => {
    if (!currentImage || !canvasRef.current || !hasMask) {
      setError('Please paint a mask over the object to remove.');
      return;
    }
    ...
    try {
        ...
        const removedImageUrl = await generateRemovedObjectImage(currentImage, maskFile);
        const newImageFile = dataURLtoFile(removedImageUrl, `removed-${Date.now()}.png`);
        addImageToHistory(newImageFile);
        handleClearMask();
    } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
  ...
  }, [currentImage, hasMask, addImageToHistory]);
  ...
  const handleMaskMouseDown = (e: React.MouseEvent | React.TouchEvent) => {
    if (activeTab !== 'remove' || isLoading) return;
  ...
  const handleMaskMouseMove = (e: React.MouseEvent | React.TouchEvent) => {
    if (!isDrawingMask.current || activeTab !== 'remove' || isLoading) return;
    const canvas = canvasRef.current;
    const ctx = canvas?.getContext('2d');
    if (!ctx) return;
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-2 backdrop-blur-sm">
            {(['retouch', 'remove', 'crop', 'adjust', 'filters'] as Tab[]).map(tab => (
                 <button
                    key={tab}
        ...
        <div className="w-full">
            ...
            {activeTab === 'remove' && <RemovePanel onApplyRemove={handleApplyRemove} onClearMask={handleClearMask} isLoading={isLoading} isMaskDrawn={hasMask} />}
